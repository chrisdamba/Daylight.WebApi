...
;(function() {
  ...
  support.Application = (function() {
    function Application($el) {
      this.$el = $el != null ? $el : $('body');
...
;(function() {
  ...
  support.Collection = (function(_super) {
    ...
    Collection.prototype.update = function(models) {
      var a, allMatch, model, o, _i, _j, _len, _len1, _results;
      if (this.length && models.length && this.length === models.length) {
        allMatch = true;
        for (_i = 0, _len = models.length; _i < _len; _i++) {
          a = models[_i];
          if (!this.find(function(b) {
        ...
        if (allMatch) {
          ...
          for (_j = 0, _len1 = models.length; _j < _len1; _j++) {
            o = models[_j];
            model = this.get(o.id);
            _results.push(model.set(model.parse(o)));
          }
          return _results;
        } else {
...
;(function() {
  ...
  support.EventAggregator = (function() {
...
;(function() {
  window.IoC = (function() {
    ...
    IoC.resetSingleton = function() {
      return this.instance(null);
    };

    IoC.init = function(config) {
    ...
    IoC.getClass = function(id) {
      return this.instance().getClass(id);
    };

    IoC.get = function(id, constructorArguments, options) {
      return this.instance().get(id, constructorArguments, options);
    };

    IoC.reset = function(id) {
      return this.instance().reset(id);
    };
    ...
    IoC.prototype.init = function(config) {
      return this._config = config;
    };

    IoC.prototype.has = function(id) {
      return this._config[id] != null;
    };

    IoC.prototype.getClass = function(id) {
      var type, _ref;
    ...
    IoC.prototype.get = function(id, constructorArguments, options) {
      ...
      if (obj) {
        ...
        if (lifestyle === 'singleton') {
          instance = this._singletons[type];
        }
        if (!instance) {
          constructor = this.getClass(id);
          properties = obj.properties;
          args = obj.args ? _.clone(obj.args) : {};
          if (constructorArguments != null) {
            _.extend(args, constructorArguments);
          }
          if (!_.size(args)) {
            args = void 0;
          }
          instance = new constructor(args, options);
          _.extend(instance, properties);
          if (lifestyle === 'singleton') {
            this._singletons[type] = instance;
          }
        }
      }
      return instance;
    ...
    IoC.prototype.reset = function(id) {
      var obj;
      obj = _config[id];
      return delete _singletons[obj.type];
    };
...
;(function() {
  ...
  support.LazyCollection = (function(_super) {
    ...
    LazyCollection.prototype.fetch = function(options) {
      ...
      return LazyCollection.__super__.fetch.call(this, options);
    };

    LazyCollection.prototype.parse = function(resp, xhr) {
      if (resp && resp.length < 1) {
        this.setNumPages(this.getPage() - 1);
      }
      return LazyCollection.__super__.parse.call(this, resp, xhr);
    };

    LazyCollection.prototype.reset = function(models, options) {
      this._page = 1;
      this._numPages = null;
      return LazyCollection.__super__.reset.call(this, models, options);
    };

    LazyCollection.prototype.getCount = function() {
      return this._count;
    };

    LazyCollection.prototype.setCount = function(value, reset) {
      if (reset == null) {
        reset = true;
      }
      this._count = value;
    ...
    LazyCollection.prototype.getPage = function() {
      return this._page;
    };

    LazyCollection.prototype.setPage = function(value) {
      return this._page = value;
    };

    LazyCollection.prototype.getNumPages = function() {
      return this._numPages;
    };

    LazyCollection.prototype.setNumPages = function(value) {
      this._numPages = value;
      if (this.getPage() > value) {
        return this.setPage(value);
      }
    };

    LazyCollection.prototype.allLoaded = function() {
      var rtn;
      return rtn = this.getNumPages() && this.getPage() === this.getNumPages() ? true : false;
    };

    LazyCollection.prototype.prevPage = function() {
      if (this._page > 1) {
        return this._page -= 1;
      }
    };

    LazyCollection.prototype.nextPage = function() {
      if (!this._numPages || this._page < this._numPages) {
        return this._page += 1;
      }
    };
...
;(function() {
  ...
  support.PopoverView = (function(_super) {
    ...
    PopoverView.prototype.initialize = function(options) {
      ...
      PopoverView.__super__.initialize.call(this, options);
      if (options.triggerEl) {
        this.$triggerEl = options.triggerEl instanceof $ ? options.triggerEl : $(options.triggerEl);
      }
      if (options.position) {
        this.position = _.extend(this.positionDefaults, options.position);
        this.position.my = this.parsePositionString(this.position.my);
        this.position.at = this.parsePositionString(this.position.at);
        this.position.offset = this.parseOffsetString(this.position.offset);
        if (this.position.of) {
          this.position.$of = this.position.of instanceof $ ? this.position.of : $(this.position.of);
        }
        if (this.position.nextTo) {
          this.position.$nextTo = this.position.nextTo instanceof $ ? this.position.nextTo : $(this.position.nextTo);
        }
      }
      return this.addListeners();
    };

    PopoverView.prototype.setElement = function(element, delegate) {
      if (this.$el) {
        this.$el.remove();
      }
      PopoverView.__super__.setElement.call(this, element, delegate);
      this.$el.addClass('popover');
      if (this.position && this.position.my) {
        this.$el.addClass(this.position.my.join('-'));
      }
      if (this.position && this.position.$nextTo) {
        this.position.$nextTo.after(this.el);
        this.updatePosition();
      } else {
        $('body').append(this.el);
      }
    ...
    PopoverView.prototype.parsePositionString = function(str) {
      var rtn;
      rtn = str.split(' ');
      if (rtn.length === 1) {
        switch (rtn[0]) {
    ...
    PopoverView.prototype.parseOffsetString = function(str) {
      var rtn;
      rtn = str.split(' ');
      if (rtn.length === 1) {
        rtn.push(rtn[0]);
      }
      rtn = _.map(rtn, function(num) {
    ...
    PopoverView.prototype.getSizeOf = function($el) {
      var rtn;
      return rtn = {
        width: $el.width(),
        height: $el.height()
    ...
    PopoverView.prototype.getPositionOf = function($el) {
      var pos, rtn, size;
      pos = $el.offset();
      size = this.getSizeOf($el);
      ...
      return rtn = _.extend(pos, size, rtn);
    };

    PopoverView.prototype.getTargetPosition = function() {
      var anchorPos, popSize, rtn, targetX, targetY;
      targetX = 0;
      targetY = 0;
      if (this.position.$of) {
        anchorPos = this.getPositionOf(this.position.$of);
        popSize = this.getSizeOf(this.$el);
        targetX = (function() {
          switch (this.position.at[0]) {
            case 'center':
              return anchorPos.xCenter;
            default:
              return anchorPos[this.position.at[0]];
          }
        }).call(this);
        targetY = (function() {
          switch (this.position.at[1]) {
            case 'center':
              return anchorPos.yCenter;
            default:
              return anchorPos[this.position.at[1]];
          }
        }).call(this);
        targetX = (function() {
          switch (this.position.my[0]) {
            case 'right':
              return targetX - popSize.width;
            case 'center':
              return targetX - (popSize.width / 2);
            default:
              return targetX;
          }
        }).call(this);
        targetY = (function() {
          switch (this.position.my[1]) {
            case 'bottom':
              return targetY - popSize.height;
            case 'center':
              return targetY - (popSize.height / 2);
            default:
              return targetY;
          }
        }).call(this);
      ...
      return rtn = {
        x: targetX + this.position.offset.x,
        y: targetY + this.position.offset.y
      };
    };

    PopoverView.prototype.updatePosition = function() {
      var targetPos;
    ...
    PopoverView.prototype.addListeners = function() {
      $(window).on('resize.popover', this.onResize);
      $(window).on('scroll.popover', this.updatePosition);
      return $(document).on('click.popover', this.onDocumentClick);
    };

    PopoverView.prototype.removeListeners = function() {
      $(window).off('resize.popover', this.onResize);
      $(window).off('scroll.popover', this.updatePosition);
      return $(document).off('click.popover', this.onDocumentClick);
    };

    PopoverView.prototype.dispose = function() {
      this.$triggerEl = null;
      this.removeListeners();
      return PopoverView.__super__.dispose.call(this);
    };

    PopoverView.prototype.close = function(data) {
      if (data == null) {
        data = null;
      }
      this.trigger('close', {
    ...
    PopoverView.prototype.onResize = function(e) {
      return this.updatePosition();
    };

    PopoverView.prototype.onDocumentClick = function(e) {
      var $popover, clickWasInPopover, clickWasOnTargetEl;
      $popover = $(e.target).closest('.popover');
      clickWasInPopover = $popover.length && $popover[0] === this.el;
      clickWasOnTargetEl = this.$triggerEl && this.$triggerEl[0] === e.target;
      if (!clickWasInPopover && (!clickWasOnTargetEl || this.options.toggle)) {
        this.close();
      }
      return true;
    };
...
;(function() {
  ...
  support.Router = (function(_super) {
...
;(function() {
  ...
  support.SyncModel = (function(_super) {
    ...
    SyncModel.prototype.isDirty = function(ignore) {
      var current, saved;
      if (ignore == null) {
        ignore = [];
      }
      ...
      current = _.omit(this.toJSON(), ignore);
      saved = _.omit(this._savedState, ignore);
      return !_.isEqual(current, saved);
    };

    SyncModel.prototype.markDirty = function() {
      return this._dirty = true;
    };

    SyncModel.prototype.markClean = function() {
